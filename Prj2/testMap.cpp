...
int main()
{
    ...
    KeyType a = "Bye";
    ValueType b = 215;
    assert(n.get(0, a, b) && a == "xyz" && b == 9876.5);
    ...
    m2.insert("Ethel", 3.734);
    m2.insert("Lucy", 1.984);

    Map m3;
    assert(!merge(m1, m2, m3) && m3.contains("Fred") && !m3.contains("Lucy") && !m3.contains("Ethel"));
    ...
    string kk;
    double vv;
    assert(gpas.get(1, kk, vv) && kk == "Ethel");
    assert(gpas.get(0, kk, vv) && kk == "");
